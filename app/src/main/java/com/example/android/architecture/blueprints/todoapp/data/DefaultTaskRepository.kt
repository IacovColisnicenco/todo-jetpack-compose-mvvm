...
class DefaultTaskRepository @Inject constructor(
    ...
    override suspend fun updateTaskOrder(tasks: List<Task>) {
        tasks.forEach { task ->
            localDataSource.updateTaskOrder(task.id, task.order)
        }
    }
    ...
    override suspend fun createTask(title: String, description: String): String {
        ...
        // Determine the order for the new task. (For Drag&Drop)
        // This is just an example; you'll likely want a more sophisticated way to determine order.
        val maxOrder = localDataSource.getAll().maxByOrNull { it.order }?.order ?: 0
